...
/* Write data to eventfd per 4 second */
void *read_thread(void *dummy)
{
	...
	for (; ; inc++) {
		...
		if (ret < 0) {
			...
			printf
			    ("0 child thread completed write %llu (0x%llx) to event_fd\n",
...
void *read_thread1(void *dummy)
{
	...
	for (; ; inc++) {
		ret = write(event_fd, &inc, sizeof(uint64_t));
		if (ret < 0) {
			perror("child thread 1 write event_fd fail.");
		} else {
			printf
			    ("1 child thread completed write %llu (0x%llx) to event_fd\n",
			     (unsigned long long)inc, (unsigned long long)inc);
		}
		sleep(8);
	}
...
int main(int argc, char *argv[])
{
	...
	ret = pthread_create(&pid1, NULL, read_thread1, NULL);
	if (ret < 0) {
		perror("pthread create fail.");
	}
			...
			for (; i < ret; i++) {
				...
				if (fd == event_fd) {
					...
					if (epollEvents & EPOLLIN) {
						ret =
						    read(event_fd, &counter,
							 sizeof(uint64_t));
						if (ret < 0) {
							printf
							    ("main thread read fail\n");
						} else {
							printf
							    ("main thread read %llu (0x%llx) from event_fd\n",
							     (unsigned long
							      long)counter,
							     (unsigned long
							      long)counter);
						...
						printf
						    ("main thread unexpected epoll events on event_fd\n");
					}
				}
			}
		} else if (ret == 0) {
